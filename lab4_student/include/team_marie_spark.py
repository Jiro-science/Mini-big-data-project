...
def transform_2(spark: SparkSession, df: DataFrame, logical_date: str) -> DataFrame:
    ...
    df = df.withColumn(
        "revenue_class",
        F.when(F.col("amount_eur") > 100, "high").otherwise("low")
    )
...
def run_daily(logical_date: str, *, with_reference: bool = False) -> dict:
    """Appelé par Airflow : enchaîne les 3 transforms et écrit les sorties."""
    spark = (
        SparkSession.builder
        .appName(f"team_marie_kpis_{logical_date}")
        .master("local[*]")
        .getOrCreate()
    )
